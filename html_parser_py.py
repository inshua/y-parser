...
def feed_string_literal(sub_state, mt, c):
...
def feed_script(sub_state2, mt, c, result):
    if c == u'*':
        mt = mt_star

    action = act_append
    new_sub_state2 = sub_state2
    ...
    if sub_state2 == sc_code:
        ...
        if mt == mt_slash:
            ...
            return not_script_close(result, mt, c)
...
def feed_style(sub_state2, mt, c, result):
    if c == u'*':
        mt = mt_star
    ...
    #         new_sub_state2 = sc_code
        ...
        if mt == mt_slash:
            ...
            return not_style_close(result, mt, c)
...
def not_script_close(result, mt, c):
...
def not_style_close(result, mt, c):
    feed_script(sc_code, mt, c, result)
    if result.action & act_undetermine:
        result.action = act_pass | act_undetermine
    else:
...
def feed_c(state, sub_state, sub_state2, c, result):
    ...
    new_state = state
    new_sub_state = sub_state
    new_sub_state2 = sub_state2
    mt = get_msg_type(c)
    action = 0
    if state == ps_text:
        if sub_state == ps_script:
            return feed_script(sub_state2, mt, c, result)
        elif sub_state == ps_style:
            return feed_style(sub_state2, mt, c, result)
...
class Parser(object):
    ...
    def feed(self, s):
        ...
        for c in s:
            ...
            if self.scanner.is_complete:
                if self.debug: print('no more match needed')
                return
    ...
    def end_curr(self, old_state, changed):
        ...
        if new_state == ps_tag_enter_children:
            ...
            if self.curr_tag in ['input','img','meta','br', 'link']:
                if self.debug: print('close tag %s' % self.span.getvalue())
                if self.until_close_depth and self.curr_depth > self.until_close_depth:
                    pass
                else:
                ...
                self.clear()
                if self.debug: print('stack %s' % (self.stack))

        if new_state == ps_comment:
            if self.debug: print('start comment')
            pass

        elif new_state == ps_dtd:
            if self.debug: print('start dtd')
            pass

        elif new_state == ps_tag_closed:
            if old_state == ps_tag_exit_children_tag_name:
                if self.debug: print('close tag have children')
                ...
                self.state = ps_text
                if self.debug: print('end comment %s' % self.span.getvalue())
                return;
            elif old_state == ps_dtd:
                self.state = ps_text
                if self.debug: print('end dtd %s' % self.span.getvalue())
                return;
            else:
                if self.debug: print('close tag %s without children' % self.span.getvalue())
                if self.until_close_depth and self.curr_depth > self.until_close_depth:
                    pass
                else:
                    if self.collect_inner_html and self.curr_depth == self.collect_inner_html:
...
def test():
    ...
    # p.feed('<h1 a=/ b=2>')
    # p.feed('<h1/><h2/>')
    # p.feed('<h1>abcd<h2>dd</h2></h1>')
    # p.feed('<!-- a -->')
    # p.feed('<!---->')
    # p.feed('<!--a--->')
    # p.feed('<!---a-->')
    # p.feed('<!doctype html>')
    # p.feed('<?xml version="1"?>')
    # p.feed('<?xml?>')
    # p.feed('<?xml ?>')
#    p.feed('<script>var a = 1 </script>')
    # p.feed('<script>var a = 1 <</script>')      # 第一个 < 处理不了
    #p.feed('<script>var a=\'abcd\'</script>')
    #p.feed('<script>var a=\'ab"cd\'</script>')
    # p.feed('<script>var a=/*comment*/\'ab"cd\'</script>')
    s = '''
        <script>
            var a = 123         // this is a <a></a>
            var d = '<script></script>'
            var c = /* ttt */ '<script>' + s + '</script>'
            var reg = /test/
            var reg = /<script>/
        </script>
    '''
...
def test3():
    ...
    selector = Selector('div', classes=['a']).child(Selector('div', classes=['b']))
    # scanner = Scanner(selector, 'value')
    # scanner = Scanner(selector, 'innerHTML')
    # scanner = Scanner(selector, 'innerText')

    # selector = Selector('div', classes=['a']).child(Selector('div', classes=['b'], total_times=2))
    selector = Selector('div', classes=['a']).descendant(Selector('div', classes=['b'], appear_times=2))
    selector = Selector('div').attr('id', 'test')
    scanner = Scanner(selector, 'innerText', debug=True)
    p = Parser(scanner)
    scanner.onfound = lambda s : print('found %s' % s)
    p.feed(s)


def test4():
    s = open('y.html', 'r', encoding='utf-8').read()
    # selector = Selector('div').attr('id', 'content').descendant(
    #                 Selector().attr('id', 'container').descendant(
    #                     Selector().attr('id', 'meta').child(
    #                         Selector().attr('id', 'title-wrapper').child(
    #                             Selector('h3').child(Selector('a').attr('id','video-title'))
    #                         )
    #                     )
    #                 ))
